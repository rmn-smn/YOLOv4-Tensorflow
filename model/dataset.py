...
from model.yolo import bbox_iou

class Dataset():
    '''
    Implements dataset for costum training data.
    code adapted from: https://github.com/pythonlessons/TensorFlow-2.x-YOLOv3

    Args: 
    dataset_type (str): "train" or "test"
    annotations (str):  path to annotations file for train or test data
    classes (str):      path to file containint classes
    batch_size (int):   batch size
    input_size (int):   size of (square) input data (input_size x input_size)
    data_augmentation:  if true, applies augmentations to image data
    load_images_to_ram: if true, loads dataset to RAM
    strides (list):     list of int denoting the yolo strides
    anchors (list):     list of int denoting the yolo anchors
    '''
    ...
    def __next__(self):
        with tf.device('/cpu:0'):
            ...
            if self.batch_count < self.num_batchs:
                ...
                if exceptions:
                    print('\n')
                    raise Exception("There were problems with dataset," 
                    "I fixed them, now restart the training process.")
                self.batch_count += 1
    ...
    def random_crop(self, image, bboxes):
        if random.random() < 0.5:
            ...
            crop_xmin = max(0, int(max_bbox[0] - 
                               random.uniform(0, max_l_trans)))
            crop_ymin = max(0, int(max_bbox[1] - 
                               random.uniform(0, max_u_trans)))
            crop_xmax = max(w, int(max_bbox[2] + 
                               random.uniform(0, max_r_trans)))
            crop_ymax = max(h, int(max_bbox[3] + 
                               random.uniform(0, max_d_trans)))
    ...
    def parse_annotation(self, annotation):
        ...
        image, bboxes = image_preprocess(
            np.copy(image), [self.input_sizes, self.input_sizes], 
            np.copy(bboxes))
        return image, bboxes
    ...
    def preprocess_true_boxes(self, bboxes):
        ...
        label = [np.zeros((
            self.train_output_sizes[i], self.train_output_sizes[i], 
        ...
        for bbox in bboxes:
            ...
            bbox_xywh = np.concatenate(
                [(bbox_coor[2:] + bbox_coor[:2]) * 0.5, bbox_coor[2:] - 
                bbox_coor[:2]], axis=-1)
            bbox_xywh_scaled = \
